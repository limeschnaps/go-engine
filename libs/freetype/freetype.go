...
import (
	...
	"github.com/limeschnaps/go-engine/libs/freetype/raster"
...
func (c *Context) StringDimensions(s string) (fixed.Point26_6, error) {
	p := Pt(0, 0)
	if c.f == nil {
		return fixed.Point26_6{}, errors.New("freetype: StringDimensions called with a nil font")
	}
	prev, hasPrev := truetype.Index(0), false
	for _, rune := range s {
		index := c.f.Index(rune)
